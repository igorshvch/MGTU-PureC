...
int main()
{
    ...
    */
    ...
    compare_cycle("C:/Users/igors/MYWRITE/PKP", "C:/Users/igors/MYWRITE/RES/RESULTS.txt");
...
void compare(char *file_path1, char *file_path2, char *output_file_path, struct words_table* wt1, struct words_table* wt2)
{
    ...
    proc = 1.00 - ((double) res)/ ((double) (wt1->len+wt2->len));
...
void compare_cycle(char *directory_path, char *output_file_path)
{
    ...
    int files_num, i, j, k, base_matr_size, base_row_size;
    char *file1, *file2;
    char **files;
    struct words_table *wt1, *wt2;
    ...
    for (i=files_num-1; i>1; i--) {
        ...
        for (k=0; k<j; k++) {
            ...
            for (i = 0; i<base_matr_size; i++)
                free((void*) wt1->matrix[i]);
            for (i = 0; i<base_matr_size; i++)
                free((void*) wt2->matrix[i]);
