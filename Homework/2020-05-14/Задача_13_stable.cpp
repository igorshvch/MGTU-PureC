...
extern void print_words_from_arr(char **_2d_arr, int rows, char *name);             //печать слов из двумерного массива
...
extern strtbl find_all_palindromes(char **_2d_arr, int rows, int *words_length)     //поиск всех палиндромов и занесение их в двумерный массив, который заносится в структуру
{
    ...
    printf("\t\tdecrement\n");
    printf("\t\tFUNC %s: rows: %d, arr_size: %d, arr_pos: %d, i: %d, count: %d\n", name, rows, arr_size, arr_pos, i, count);

    arr_size++;
    pal_arr = (char**) realloc(pal_arr, sizeof(char*)*arr_size);

    printf("\t\tFUNC %s: rows: %d, arr_size: %d, arr_pos: %d, i: %d, count: %d\n", name, rows, arr_size, arr_pos, i, count);

    for (i=0; i<arr_size; i++)
        printf("ID: %d NAME: %s, ADDRESS: %p, string: %s\n", i, name, pal_arr[i], pal_arr[i]);
    
    //printf("ID: %d NAME: %s, ADDRESS: %p, string: %s\n", i, name, pal_arr[i+1], pal_arr[i+1]);
    arr_size--;

    if (count != rows-1)               //если последнее слово не палиндром, то уменьшаем 
        arr_size--;
...
int is_palindrome(char *word, int length)                                           //проверка слова на палинром
{
    int i, j;
...
void print_words_from_arr(char **_2d_arr, int rows, char *name)                     //печать слов из двумерного массива
{
    int i;
